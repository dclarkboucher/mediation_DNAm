...
for(r in 1:nrow(tab)){
  ...
  # Process univariate
  out_uni <- read_rds(filename("uni", seed, dataset))
  uni <-
    out_uni |>
    rename(pv = ab_pv) |> 
  ...
  # Process HIMA
  out_hima <- read_rds(filename("hima",seed,dataset))
  hima <-
    out_hima |>
  ...
  # Process HDMA
  out_hdma <- read_rds(filename("hdma", seed,dataset))
  hdma <-
    out_hdma |>
    rownames_to_column("mediator") |>
    transmute(
  ...
  # Process MedFix
  out_med <- read_rds(filename("med", seed,dataset))
  med <-
    out_med |>
    transmute(
      mediator = variable,
  ...
  # Process BSLMM
  out_bslmm <- read_rds(filename("bslmm", seed, dataset))
  bslmm <- 
    out_bslmm |> 
    transmute(
  ...
  load(filename("plasso", seed, dataset))
  ...
  # HILMA
  load(filename("hilma", seed, dataset))
  hilma <-
    tibble(method = "HILMA",
           tie = with(d, sum(ab)),
  ...
  # PCMA
  load(filename("pcma", seed, dataset))
  pcma <-
    tibble(method = "PCMA",
           tie = with(d, sum(ab)),
  ...
  # PMED
  load(filename("pmed", seed, dataset))
  pmed <-
    tibble(method = "PMED",
           tie = with(d, sum(ab)),
           tie_hat = pmed_tie,
  ...
  results[[r]] <-
    ...
    mutate(dataset = dataset, seed = seed) |> 
    select(dataset, seed, method, everything())
...
dat.sum <-
  ...
  summarize(
    across(c(tpr, fpr, mse0, mse1, rmse0, rmse1, tie_prb), 
           list(
             m = mean,
...
write_rds(dat.sum, "results/positive_effects_results.rds")
